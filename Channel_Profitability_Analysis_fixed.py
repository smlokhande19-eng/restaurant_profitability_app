...
def main():
    ...
    file_path = r"E:\github\restuarnt_profitabilit_app\SkyCity Auckland Restaurants & Bars.csv"
    ...
    if commission_adjustment != 0 or delivery_cost_adjustment != 0:
        ...
        if commission_adjustment != 0:
            # Recalculate UberEats and DoorDash profit with adjusted commission
        ...
        if delivery_cost_adjustment != 0:
            # Recalculate Self-Delivery profit with adjusted costs
    ...
    with tab1:
        ...
        with col1:
            st.metric(
                ...
                help="Total revenue across all channels"
            )
        ...
        with col2:
            st.metric(
                ...
                help="Total net profit across all channels"
            )
        
        with col3:
            overall_margin = (kpis['total_profit'] / kpis['total_revenue'] * 100)
            st.metric(
                "Overall Margin",
                f"{overall_margin:.1f}%",
                help="Overall profit margin"
            )
        ...
        with col4:
            st.metric(
                "Restaurants",
                len(filtered_df),
                help="Number of restaurants in filtered dataset"
            )
        ...
        with col1:
            ...
            fig = px.pie(
                ...
                values='Orders',
                names='Channel',
                title='Orders Distribution by Channel',
    ...
    with tab2:
        ...
        with col2:
            fig = px.bar(
                ...
                x='Channel',
                y='Profit per Order',
                title='Profit per Order by Channel',
        ...
        fig.add_trace(go.Bar(
            ...
            y=channel_data['Margin (%)'],
            text=channel_data['Margin (%)'].round(1),
            texttemplate='%{text}%',
            textposition='outside',
            marker_color=['#2ecc71', '#e74c3c', '#e67e22', '#3498db']
        ))
    ...
    with tab3:
        ...
        # Waterfall chart for each channel
        st.markdown("### Revenue to Profit Waterfall")
        
        selected_waterfall_channel = st.selectbox(
            "Select Channel for Waterfall Analysis",
        ...
        # Calculate commission or delivery costs
        if selected_waterfall_channel in ['Uber Eats', 'DoorDash']:
            ...
            commission = 0
            if selected_waterfall_channel == 'Self-Delivery':
                delivery_cost = filtered_df['SD_DeliveryTotalCost'].sum()
            else:
                delivery_cost = 0
        ...
        fig.update_layout(
            title=f"{selected_waterfall_channel} - Revenue to Profit Breakdown",
            showlegend=False,
            height=500
        )
        ...
        # Cost structure comparison across channels
        ...
        for channel in ['In-Store', 'Uber Eats', 'DoorDash', 'Self-Delivery']:
            ...
            if rev > 0:
                ...
                cost_structure_data.append({
                    ...
                    'COGS %': cogs_pct,
                    'OPEX %': opex_pct,
                    'Commission/Delivery %': commission_pct + other_pct,
    ...
    with tab4:
        ...
        commission_data = filtered_df[
            (filtered_df['UberEatsRevenue'] > 0) | (filtered_df['DoorDashRevenue'] > 0)
        ].copy()
        ...
        with col1:
            st.metric(
                ...
                f"${commission_data['TotalCommission'].sum():,.0f}",
                help="Total commission paid to third-party platforms"
            )
        ...
        with col2:
            ...
            st.metric(
                ...
                f"{commission_rate_avg:.1f}%",
                help="Average commission as % of delivery revenue"
            )
        
        # Commission by segment
        commission_by_segment = commission_data.groupby('Segment').agg({
            'TotalCommission': 'sum',
            'UberEatsRevenue': 'sum',
            'DoorDashRevenue': 'sum'
        }).reset_index()
        
        commission_by_segment['CommissionRate'] = (
            commission_by_segment['TotalCommission'] / 
            (commission_by_segment['UberEatsRevenue'] + commission_by_segment['DoorDashRevenue']) * 100
        )
        
        fig = px.bar(
            commission_by_segment,
            x='Segment',
            y='TotalCommission',
            title='Total Commission Paid by Segment',
            color='CommissionRate',
        ...
        with col1:
            st.metric(
                ...
                f"${delivery_data['SD_DeliveryTotalCost'].sum():,.0f}",
                help="Total cost for self-managed delivery"
            )
        ...
        with col2:
            ...
            st.metric(
                ...
                f"${avg_cost_per_order:.2f}",
                help="Average cost per self-delivery order"
            )
        
        # Delivery cost by cuisine
        delivery_by_cuisine = delivery_data.groupby('CuisineType').agg({
            'SD_DeliveryTotalCost': 'sum',
            'SelfDeliveryOrders': 'sum',
            'DeliveryCostPerOrder': 'mean'
        }).reset_index()
        ...
        fig = px.bar(
            ...
            title='Self-Delivery Costs by Cuisine (Top 10)',
            color='DeliveryCostPerOrder',
            color_continuous_scale='Blues',
            text='SD_DeliveryTotalCost'
        )
        fig.update_traces(texttemplate='$%{text:,.0f}', textposition='outside')
        ...
        efficiency_data['TotalRevenue'] = (
            efficiency_data['InStoreRevenue'] + 
            efficiency_data['UberEatsRevenue'] + 
            efficiency_data['DoorDashRevenue'] + 
            efficiency_data['SelfDeliveryRevenue']
        )
        efficiency_data['TotalProfit'] = (
            efficiency_data['InStoreNetProfit'] + 
            efficiency_data['UberEatsNetProfit'] + 
            efficiency_data['DoorDashNetProfit'] + 
            efficiency_data['SelfDeliveryNetProfit']
        )
        ...
        fig = px.scatter(
            ...
            title='Revenue vs Margin % by Restaurant',
            labels={'TotalRevenue': 'Total Revenue ($)', 'MarginPct': 'Profit Margin (%)'}
        )
    ...
    with tab5:
        ...
        # Profitability heatmap by cuisine and segment
        st.markdown("### Profitability Heatmap: Cuisine vs Segment")
        
        heatmap_channel = st.selectbox(
            "Select Channel for Heatmap",
            ['In-Store', 'Uber Eats', 'DoorDash', 'Self-Delivery'],
            key='heatmap_channel'
        )
        ...
        heatmap_data = filtered_df.groupby(['CuisineType', 'Segment']).agg({
            channel_revenue_col: 'sum',
            channel_profit_col: 'sum'
        }).reset_index()
        ...
        fig = go.Figure(data=go.Heatmap(
            ...
            colorscale='RdYlGn',
            text=pivot_margin.values.round(1),
            texttemplate='%{text}%',
            textfont={"size": 10},
        ...
        fig.update_layout(
            title=f'{heatmap_channel} - Profit Margin by Cuisine & Segment',
            xaxis_title='Segment',
        ...
        with col1:
            ...
            top_performers['TotalProfit'] = (
                top_performers['InStoreNetProfit'] + 
                top_performers['UberEatsNetProfit'] + 
                top_performers['DoorDashNetProfit'] + 
                top_performers['SelfDeliveryNetProfit']
            )
            
            top_5 = top_performers.nlargest(5, 'TotalProfit')[
                ['RestaurantName', 'CuisineType', 'Segment', 'TotalProfit']
            ]
        ...
        with col2:
            ...
            bottom_5 = top_performers.nsmallest(5, 'TotalProfit')[
                ['RestaurantName', 'CuisineType', 'Segment', 'TotalProfit']
            ]
        ...
        fig = px.bar(
            ...
            color='Channel',
            barmode='group',
            title='Profit by Segment & Channel',
            text='Profit'
        )
        fig.update_traces(texttemplate='$%{text:,.0f}', textposition='outside')
        st.plotly_chart(fig, use_container_width=True)
    ...
    with tab6:
        ...
        risk_channel = st.selectbox(
            "Select Channel for Risk Analysis",
            ['In-Store', 'Uber Eats', 'DoorDash', 'Self-Delivery'],
            key='risk_channel'
        )
        ...
        fig = px.histogram(
            filtered_df,
            x=risk_profit_col,
            title=f'{risk_channel} - Profit Distribution',
            labels={risk_profit_col: 'Net Profit ($)'},
            nbins=30,
            marginal='box'
        )
        
        fig.add_vline(
            x=filtered_df[risk_profit_col].mean(),
            line_dash="dash",
            line_color="red",
        ...
        for cuisine in filtered_df['CuisineType'].unique():
            ...
            risk_by_cuisine.append({
                'Cuisine': cuisine,
                'Mean Profit': mean_val,
                'Volatility': cv_val,
                'Count': len(cuisine_df)
            })
        ...
        fig = px.scatter(
            ...
            size='Count',
            color='Cuisine',
            title=f'{risk_channel} - Risk-Return Profile by Cuisine',
            labels={'Mean Profit': 'Average Profit ($)', 'Volatility': 'Volatility (CV %)'},
            hover_data=['Count']
        )
        
        st.plotly_chart(fig, width='stretch')
        
        # Loss-prone restaurants
        ...
        if len(high_risk) > 0:
            high_risk['TotalProfit'] = (
                high_risk['InStoreNetProfit'] + 
                high_risk['UberEatsNetProfit'] + 
                high_risk['DoorDashNetProfit'] + 
                high_risk['SelfDeliveryNetProfit']
            )
            
            high_risk_display = high_risk.nsmallest(10, 'TotalProfit')[
                ['RestaurantName', 'CuisineType', 'Segment', 'LossChannels', 'TotalProfit']
            ].copy()
